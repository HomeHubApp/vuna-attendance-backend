/**
 * @file Admin's user-management operations: list every user (students,
 * staff, admins) with their roles and department, update a user's own
 * fields, and delete one.
 *
 * @remarks
 * Lives under `admin/` because every one of these operations is Admin-only
 * (guarded in `adminUserRoutes.js`) — it is not shared with any other role,
 * unlike `shared/*Service.js` files. `getAllUsers` fetches every user and
 * paginates in memory rather than in the query; fine at this institution's
 * scale, but the first thing to change if the user list gets large enough
 * to matter.
 */

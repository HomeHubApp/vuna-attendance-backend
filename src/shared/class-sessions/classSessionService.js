/**
 * @file The live "class session" feature: a lecturer starting and ending
 * one dated occurrence of a scheduled class, and both a lecturer and a
 * student finding which sessions are currently active.
 *
 * @remarks
 * Lives in `shared/` because `lecturer/class-sessions/classSessionRoutes.js`
 * (start/end/active/mine) and `student/class-sessions/classSessionRoutes.js`
 * (`getActiveSessionsForStudent`) both call into this one service — a
 * `class_sessions` row is the same data whichever role is reading it.
 */

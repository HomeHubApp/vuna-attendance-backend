/**
 * @file Notifications: creating one event and fanning it out to every
 * recipient, and a user's own list, unread count and mark-as-read.
 *
 * @remarks
 * Every role reads its own notifications through the same endpoints
 * (`shared/notifications/notificationRoutes.js`), so the whole feature —
 * routes, controller and this service — lives in `shared/` rather than
 * under any one role. Other features that create notifications (for
 * example `shared/class-schedule/classScheduleNotifications.js`) call
 * `NotificationService.notify(...)` without knowing how they're stored.
 */

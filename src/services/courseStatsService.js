...
// lists overflow the request URL, so bulk reads go through these two helpers.
...
async function fetchAllPages(buildQuery, fallbackMessage) {
  const rows = [];
...
async function fetchInChunks(ids, buildQuery, fallbackMessage) {
  const rows = [];
